...
def generate_data(n):
    ...
    rd.seed(42)
    data = []
    nomes_aleatorios = nome_aleatorio.gerar_nomes_aleatorios(n, 42)
    for i in range(n):
...
def gen_hashtable_samples(num_elements, **kwargs):
    ...
    p_dense = (
        p_hat_dense  # Proporção de elementos densos da lista: quanto menor, mais denso
    )
    ...
    sparse_range_end = (
        int((num_elements - dense_range_size) ** 2)
        if final_range <= (sparse_range_start + num_sparse_keys)
        else final_range
    )

    # Gera as chaves aleatórias
    dense_keys = rd.sample(
        range(start_range, start_range + dense_range_size), num_dense_keys
    )
    sparse_keys = rd.sample(
        range(sparse_range_start, sparse_range_end + 1), num_sparse_keys
    )
